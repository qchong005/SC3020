...
struct BPlusNode {
    ...
    BPlusNode(bool leaf = false) : is_leaf(leaf), is_root(false), node_id(0),
                                   next_leaf(0), parent_id(0) {}
};
...
public:
    ...
    int getTreeLevels() const { return tree_height; }
    std::vector<float> getRootKeys() const;

    void printStatistics();
    void printTree(); // Debug function
